...
public class PlusSetup extends permeagility.plus.PlusSetup {
	...
	public String getAddForm(DatabaseConnection con) {
		return "Add to menu"+createListFromTable("MENU_"+getPackage(), "", con, "menu")
				+"<br>Roles: "
				+linkListControl(con, "ROLES_"+getPackage(), "identity", 
						getCache().getResult(con,getQueryForTable(con, "identity")), con.getLocale(), null);
	}
	...
	@Override public boolean install(DatabaseConnection con, HashMap<String,String> parms, StringBuilder errors) {
		...
		// No tables
                ...
                // Menu items
		Setup.createMenuItem(con,"ImportJSON","Import JSON into a table",MENU_CLASS_IMP,parms.get("MENU_"+getPackage()),parms.get("ROLES_"+getPackage()));	
		Setup.createMenuItem(con,"ExportJSON","Export JSON from table or SQL",MENU_CLASS_EXP,parms.get("MENU_"+getPackage()),parms.get("ROLES_"+getPackage()));	
		Setup.createMenuItem(con,"JSON Download","Download JSON data",MENU_CLASS_DATA,null,parms.get("ROLES_"+getPackage()));	// Not on the menu
